...
class Banned_Plugins {
	...
	public function __construct() {
		...
		if ( self::shall_display_admin_notice() && version_compare( $wp_version, '4.3', '>' ) ) {
			...
			add_action( 'wp_ajax_wa_dismiss_banned_plugins_nag', array( $this, 'dismiss_banned_plugins_nag' ), PHP_INT_MAX );
		}
	}
	...
	public function admin_notices() {
		...
		?>
		<div id="WA-banned-plugins-nag" class="notice notice-WA notice-error is-dismissible">
			<div class="notice-WA__content">
	...
	public function warning_plugin_action_links( $actions ) {
		...
		if ( is_string( $banned_action_link ) && ! empty( $banned_action_link ) ) {
			$warning_actions = array_merge( array( 'wa_banned' => $banned_action_link ), $warning_actions );
		}
	...
	public function disabled_plugin_action_links( $actions ) {
		...
		$banned_action_link = $this->get_banned_plugin_action_link();
		if ( is_string( $banned_action_link ) && ! empty( $banned_action_link ) ) {
			$disabled_actions = array_merge( array( 'wa_banned' => $banned_action_link ), $disabled_actions );
		}
	...
	public function plugin_install_action_links( $action_links, $plugin ) {
		...
		foreach ( $this->banned_list as $banned ) {
			...
			if ( $plugin['slug'] === $banned_slug ) {
				...
				$action_links = array(
					'<button type="button" class="button button-disabled" disabled="disabled">' . __( 'Banned', 'wa-mu-plugins' ) . '</button>',
				);

				$banned_action_link = $this->get_banned_plugin_install_action_link();
				if ( is_string( $banned_action_link ) && ! empty( $banned_action_link ) ) {
					$action_links = array_merge( $action_links, array( 'wa_banned_why' => $banned_action_link ) );
				}
			}
		}
	...
	public function install_plugin_complete_actions( $install_actions, $api, $plugin_file ) {
		...
		if ( in_array( $plugin_file, $this->banned_list, true ) ) {
			...
			$install_actions = array(
				'wa_banned_plugin' => '<button type="button" class="button button-disabled" disabled="disabled">' . __( 'Banned', 'wa-mu-plugins' ) . '</button>',
			);

			$banned_action_link = $this->get_banned_plugin_install_action_link();
			if ( is_string( $banned_action_link ) && ! empty( $banned_action_link ) ) {
				$install_actions = array_merge( $install_actions, array( 'wa_banned_why' => $banned_action_link ) );
			}
		}
	...
	public function dismiss_banned_plugins_nag() {
		check_ajax_referer( 'wa-banned-plugins', 'nonce' );
		set_transient( 'wa_dismiss_banned_plugins_nag', '1', MONTH_IN_SECONDS );
		wp_die();
	}
	...
	private function get_the_admin_notice_content() {
		...
		$heading = _n( 'WA detected a banned plugin', 'WA detected banned plugins', count( $notifiable_plugins ), 'wa-mu-plugins' );

		// Translators: 1st %s the "heading" (e.g. WA detected a banned plugin), 2nd %s plugin names.
		...
		$notice_content .= '<p>' . sprintf( __( 'Please deactivate %s as soon as possible. Using a banned plugin can cause performance issues for your site or break wordpress due to poor coding of the plugin.', 'wa-mu-plugins' ), _n( 'this plugin', 'these plugins', count( $notifiable_plugins ), 'wa-mu-plugins' ) ) . '</p>';

		// The button.
		$notice_content .= '<p><a class="button action" href="https://webactive.io/banned-plugins/" target="_blank">' . __( 'Learn more about banned plugins', 'wa-mu-plugins' ) . '<span class="dashicons dashicons-external"></span></a></p>';

		// Set the default string when WAMU_WHITELABLE is enabled.
		if ( is_whitelabel_enabled() ) {

			$heading = _n( 'Banned plugin detected', 'Banned plugins detected', count( $notifiable_plugins ), 'wa-mu-plugins' );

			// Translators: 1st %s the "heading" (e.g. WA detected a banned plugin), 2nd %s plugin names.
			$notice_content = '<p>' . sprintf( '<strong>%s</strong>: %s.', $heading, implode( ', ', $notifiable_plugins ) ) . '</p>';

			$notice_content .= '<p>' . __( 'Using a banned plugin can cause performance issues for your site or break wordpress due to poor coding of the plugin.', 'wa-mu-plugins' ) . '</p>';

			// Translators: %s "this plugin" if singular, "these plugins" if plural.
			$notice_content .= '<p>' . sprintf( __( 'Please deactivate %s as soon as possible.', 'wa-mu-plugins' ), _n( 'this plugin', 'these plugins', count( $notifiable_plugins ), 'wa-mu-plugins' ) ) . '</p>';
		}

		return apply_filters( 'wa_banned_plugin_notice_content', $notice_content, $notifiable_plugins );
	}
	...
	public function get_banned_plugin_action_link() {
		...
		$banned_action = '<a class="WA-plugin-action WA-plugin-action--banned" href="https://webactive.io/banned-plugins/" target="_blank">' . __( 'Banned', 'wa-mu-plugins' ) . ' <span class="dashicons dashicons-external"></span></a>';

		if ( is_whitelabel_enabled() ) {
			$banned_action = '<span class="WA-plugin-action WA-plugin-action--banned">' . __( 'Banned', 'wa-mu-plugins' ) . '</span>';
		}

		return apply_filters( 'wa_banned_plugin_action_link', $banned_action );
	}
	...
	public function get_banned_plugin_install_action_link() {
		...
		$banned_action = '<a href="https://webactive.io/banned-plugins/" target="_blank">' . __( 'Why?', 'wa-mu-plugins' ) . '</a>';
		if ( is_whitelabel_enabled() ) {
			$banned_action = '';
		}

		return apply_filters( 'wa_banned_plugin_install_action_link', $banned_action );
	}
	...
	private function get_list_tables_styles() {
		...
		.plugins .WA-plugin-action--banned .dashicons {
			float: none;
			width: 16px;
			height: 16px;
			padding: 0;
		...
		.plugins .WA-plugin-action--banned .dashicons:before {
			color: inherit;
			background: none;
			box-shadow: none;
			font-size: 16px;
		...
		.WA-banned-plugin .WA-banned-plugin__title {
			color: #0073aa;
		}
	...
	public function add_plugin_page_scripts() {
		...
		if ( self::is_admin_plugin_page() ) {
			...
			$warning_data = 'var WAWarningPlugins = ' . json_encode( $warning_slugs ) . ';';
			...
			$disabled_data = 'var WADisabledPlugins = ' . json_encode( $disabled_slugs ) . ';';
	...
	.notice-WA__content p {
		margin: 0 0 2px;
		padding: 0;
	}
	.notice-WA__content p:last-child {
		margin: 0;
	}
	.notice-WA__content .button {
		margin-top: 8px;
	}
	.notice-WA__content .button .dashicons {
		position: relative;
		top: 5px;
		margin-left: 5px;
	}
	.notice-WA__content .button .dashicons,
	.notice-WA__content .button .dashicons-before:before {
		width: 16px;
		height: 16px;
		font-size: 16px;
	}
	...
	if ( typeof jQuery !== 'undefined' ) {
		jQuery( function( $ ) {
			$( '#wpbody-content' ).on( 'click', '#wa-banned-plugins-nag .notice-dismiss', function() {
				$.post( ajaxurl, {
					nonce: '<?php echo esc_attr( wp_create_nonce( 'wa-banned-plugins' ) ); ?>',
					action: 'wa_dismiss_banned_plugins_nag'
				}, function( response ) {
	...
	private function shall_display_admin_notice() {
		...
		$shall = false;
		$dismissed = get_transient( 'wa_dismiss_banned_plugins_nag' );
		if ( ! function_exists( 'get_plugins' ) ) {
			require_once ABSPATH . 'wp-admin/includes/plugin.php';
		}
		$installed_plugins = get_plugins();
	...
	private function check_server_banned_plugin_lists() {
		...
		if ( isset( $_SERVER ) && isset( $_SERVER['WA_BANNED_PLUGINS'] ) ) {
			$banned_plugins_array = json_decode( $_SERVER['WA_BANNED_PLUGINS'], $assoc_array = true );
